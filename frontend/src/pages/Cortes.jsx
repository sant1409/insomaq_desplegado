...
export default function Cortes() {
  ...
  const fetchRetazos = async () => {
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/retazos`);
  ...
  const fetchUsuarios = async () => {
    try {
      ...
      let res = await fetch(`${process.env.REACT_APP_API_URL}/usuarios/public`);
      ...
      if ((res.status === 404 || res.status === 401 || res.status === 403) && token) {
        res = await fetch(`${process.env.REACT_APP_API_URL}/usuarios`, { headers: { Authorization: `Bearer ${token}` } });
      }
